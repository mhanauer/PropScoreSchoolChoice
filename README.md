...
```
Need descriptives for matched data.  Do this for public and then private
```{r}
mean_sd_public = NULL
for(i in 1:m){
  mean_sd_public[[i]] = subset(datAll[[i]], X1PUBPRI == 0)
}
lapply(mean_sd_public, function(x) describe(x))
datAllPublic = lapply(datAll, function(x) subset(datAll[[x]], X1PUBPRI == 0))
datAllPublic = lapply(datAll, function(x) describe(x))
