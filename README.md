...
# need to grab the n for each data set and then get the mean.  This for the imputed prematch n
n_m_out = NULL
for(i in 1:m){
  n_m_out[[i]] = dim(m.out[[i]])
  n_m_out[[i]] = n_m_out[[i]][[1]]
}
n_m_out = data.frame(n_m_out); n_m_out
n_m_out = apply(n_m_out, 1, mean); n_m_out
```
